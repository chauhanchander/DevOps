...
csingh1@cvluawsbisam1:/home/csingh1/EDMIK-275-NEW/NEW-REQUIREMENT> cat space_check.sh
#!/bin/bash -x
...
if  [ "$SIZE_USED_PERCENTAGE" -gt "$THRESHOLD" ];then
    ...
    CHECK_SPACE=`"$PSQL" -h "$SERVER_NAME" -p "$SERVER_PORT" -U "$USER_NAME" -d "$DB_NAME" --file="$PWD/spacecheck.sql" > "$LOGS_DIR/$1.sqloutput"`
    TOTAL_SIZE_OF_SCHEMA=`"$PSQL" -h "$SERVER_NAME" -p "$SERVER_PORT" -U "$USER_NAME" -d "$DB_NAME" --file="$PWD/schemasizecheck.sql" > "$LOGS_DIR/$1.$6.schemasize"`
    REMOVE_TOP_LINES_OF_SCHEMA=`$CAT "$LOGS_DIR/$1.$6.schemasize"|$SED '1,2d'|$SED '/^\s*$/d'|$AWK '{print $3}' > "$LOGS_DIR/$1.$6.schemasize.clean"`
    TOTAL_SIZE_OF_SCHEMA=`$CAT "$LOGS_DIR/$1.$6.schemasize.clean"`
...
#  Creating CSV file
  ...
  CREATE_TEXT_FILE=`$PASTE "$LOGS_DIR/$1.sqloutput.updated" "$LOGS_DIR/$1.tablename" "$LOGS_DIR/$1.schemaname" "$LOGS_DIR/$1.tablespaceingb" "$LOGS_DIR/$1.tablespaceinbytes" |$COLUMN -s $'\t' -t > "$LOGS_DIR/$1.table_information.txt"`
    ...
    do
       ...
       INSERT_SCHEMA_SIZE=`"$ECHO" ", ,Schema_Size,${TOTAL_SIZE_OF_SCHEMA}" >> "$LOGS_DIR/${myRecArray[1]}.${1}.report.csv"`
