...
select
   (sum(capacity)/1024) - (0.26 * (sum(capacity)/1024)) as capacity_GBytes,
   (sum(used)/1024) as used_GBytes ,
   round((sum(used)/1024)/((sum(capacity)/1024) - (0.26 * (sum(capacity)/1024))),2)*100 as used_percentage,
   (sum(capacity)/1024) - (0.26 * (sum(capacity)/1024)) - (sum(used)/1024) as Free_Space
...
[[ ${SIZE_USED_PERCENTAGE} -gt 100 ]] && SIZE_USED_PERCENTAGE=100
    ...
    do
       ...
       REMOVE_FROM_FILE=`${SED} -i "1,${myRecArray[0]}d" "$LOGS_DIR/$1.table_information.txt"`
        ...
        if [ ${TOTAL_SIZE_OF_SCHEMA} ]; then
            SEND_EMAIL_TO_USER=`"$PWD/email_users_for_space.sh"  "$1" "$4" "$6" "$LOGS_DIR/$1.table_information.csv" "$LOGS_DIR/${myRecArray[1]}.$1.report.csv" "${myRecArray[1]}" "${TOTAL_SIZE_OF_SCHEMA}" "${SIZE_USED_PERCENTAGE}"`
        else
