...
kill_on_db() {
    ...
    OUTPUT=`${PG_CON} -t -c "select pid from stv_inflight where lower(trim(text)) like 'vacuum%'"|tr -d " "`
    if [[ -z ${OUTPUT} ]]; then
        ...
        ${PG_CON} -c "select pg_terminate_backend(${OUTPUT})" 2>${ERROR_FILE} 1>/dev/null
        if [[ -s ${ERROR_FILE} ]]; then
            ...
            msg "INFO" "Process ID ${OUTPUT} terminated successfully."
        fi
    fi
}
...
kill_on_os() {
    ...
    if [[ -z ${PID} ]]; then
        ...
        msg "INFO" "`ps -aef | grep -w "rs_vacuum_tables.sh ${RS_CLUSTER}"| grep -v grep`"
        for p_id in ${PID}; do
