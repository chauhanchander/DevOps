...
OUTPUT_DIR=postgres_stack_provision_output/${SERVER1_HOSTNAME}_${DATETIME}
...
cp postgres_stack_provision.yml ${OUTPUT_DIR}/
...
ansible-playbook -i ${HOSTFILE} postgres_stack_provision.yml --vault-password-file ~/.postgres_vault_pass.txt --extra-vars "@${INPUT_FILE}" $* |tee ${OUTPUT_DIR}/playbook.out
