select
    ...
    coalesce(t.blocks / 1024,0)::bigint as "Total GB",
    ...
    case
        when t.max_rows > t.min_rows then (t.max_rows - t.min_rows)::numeric(18,5) / t.max_rows
            when t.max_rows > t.min_rows and t.min_rows = 0 then 1::numeric(18,5)
            else 0
            ...
            join pg_catalog.pg_namespace s on s.oid = c.relnamespace and s.nspname not in ('pg_catalog', 'information_schema', 'pg_toast', 'pg_bitmapindex', 'pg_internal', 'pg_aoseg')
            ...
            ) b on b.tbl = p.id and b.slice = p.slice
            group by p.id, p.db_id
            ) t on t.id = c.oid
            where c.relkind = 'r'
                        and s.nspname = 'workmsmgr'
            order by u.usename, coalesce(t.blocks / 1024,0)::bigint desc
